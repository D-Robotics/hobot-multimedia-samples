...
MIPI_SENSOR_INFO_S SENSOR_2LANE_SC230AI_30FPS_10BIT_LINEAR_INFO =
{
	...
	.sensorInfo = {
		...
		.bus_num = 1, // 总线号，根据具体板子硬件原理图确定 , 不配置默认 i2c5 ，对应host2
...
VIN_DEV_ATTR_S DEV_ATTR_SC230AI_LINEAR_BASE = {
	...
	.mipiAttr = { // sif(dev) 输入的接口模式， mipi or dvp,目前都是 mipi
		...
		.ipi_mode = 0, // 当 DOL2 分成两路 linear 或者 DOL3 分成一路 DOl2 和一路 linear 或者三路linear 的时候，此值就赋值为 2 或 3.
		.enable_mux_out = 1, // 未用
...
static int set_sensor_param(void)
{
	...
	/*定义 sensor   初始化的属性信息 */
	snsinfo = SENSOR_2LANE_SC230AI_30FPS_10BIT_LINEAR_INFO;
	/*定义 mipi 初始化参数信息 */
	mipi_attr = MIPI_2LANE_SENSOR_SC230AI_30FPS_10BIT_LINEAR_ATTR;
	/*定义 dev 初始化的属性信息 */
	devinfo = DEV_ATTR_SC230AI_LINEAR_BASE;
	/*定义 pipe 属性信息 */
	pipeinfo = PIPE_ATTR_SC230AI_LINEAR_BASE;
	/*定义 dis 属性信息 */
	disinfo = DIS_ATTR_SC230AI_LINEAR_BASE;
	/*定义 ldc 属性信息 */
	ldcinfo = LDC_ATTR_SC230AI_LINEAR_BASE;
