...
#define SENSOR_TAG "SC230AI_DOL2"

#define F_WIDTH 1920
#define F_HEIGHT 1080

MIPI_SENSOR_INFO_S SENSOR_2LANE_SC230AI_30FPS_10BIT_DOL2_INFO =
{
	...
	.sensorInfo = {
		...
		.sensor_mode = DOL2_M, // sensor 工作模式， 1 是 normal,2 是dol2,3 是 dol3
...
MIPI_ATTR_S MIPI_2LANE_SENSOR_SC230AI_30FPS_10BIT_DOL2_ATTR =
{
	.mipi_host_cfg = {
		.lane = 2, // 硬件上sensor用了几个mipi数据lane，f37用 1 lane, imx415 4K 用 4 lane
		...
		.height = F_HEIGHT,
		.linelenth = 2250, // sensor 输出带 blanking 的总行长， 一般根据sensor供应商的配置参数配置就行
		.framelenth = 2200, // sensor 输出带 blanking的总行数
		.settle = 30, // sensor 输出实际Ttx-zero + Ttx-prepare时间（ clk 为单位）, 一般0-100之间尝试
		.channel_num = 2, // 使用虚通道的个数, linear 用一个，dol2 用 2 个， dol3 用3个
		.channel_sel = {0,1} // 保存每个虚通道的值，在调用HB_VIN_SetDevVCNumber接口时用第一个，调用HB_VIN_AddDevVCNumber用后面的2 3 4的配置
	},
	.dev_enable = 0 // mipi dev 是否使能， 1是使能， 0 是关闭, 只用开启mipi bypass的开启
};


VIN_DEV_ATTR_S DEV_ATTR_SC230AI_DOL2_BASE = {
	.stSize = { // 输入的数据
	...
	.mipiAttr = { // sif(dev) 输入的接口模式， mipi or dvp,目前都是 mipi
		...
		.ipi_channels = 2, // sif和isp之间的硬件通道，可以把多个mipi虚拟通道合并到这一个ipi channel中，ipi_channels 表示用了几个 channel ，默认是 0 开始，如果设置是 2 ，是用了 0 ，1
		.ipi_mode = 2, // 当 DOL2 分成两路 linear 或者 DOL3 分成一路 DOl2 和一路 linear 或者三路linear 的时候，此值就赋值为 2 或 3.
		.enable_mux_out = 1, // 未用
	...
	.outIspAttr = { // sif 到 isp 一些属性设置
		.dol_exp_num = 2, // 曝光模式， 1 为普通模式， dol 2 或者 3设置对应数目
...
VIN_PIPE_ATTR_S PIPE_ATTR_SC230AI_DOL2_BASE = {
...
static int set_sensor_param(void)
{
	...
	/*定义 sensor   初始化的属性信息 */
	snsinfo = SENSOR_2LANE_SC230AI_30FPS_10BIT_DOL2_INFO;
	/*定义 mipi 初始化参数信息 */
	mipi_attr = MIPI_2LANE_SENSOR_SC230AI_30FPS_10BIT_DOL2_ATTR;
	/*定义 dev 初始化的属性信息 */
	devinfo = DEV_ATTR_SC230AI_DOL2_BASE;
	/*定义 pipe 属性信息 */
	pipeinfo = PIPE_ATTR_SC230AI_DOL2_BASE;
