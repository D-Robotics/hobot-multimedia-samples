...
int main(int argc, char **argv) {
    int s32Ret;
    int opt = 0;
    SAMPLE_VENC_ATTR_S sample_attr, sample_attr1;
    ...
    while((opt = getopt(argc,argv,"w:h:t:f:g:")) != -1) {
        switch(opt) {
            ...
            case 'f':
                ...
                sample_attr1.picFile0 = sample_attr.picFile0;
                break;
            case 'g':
                sample_attr.picFile1 = optarg;
                sample_attr1.picFile1 = sample_attr.picFile1;
                break;
            default:
                print_usage();
                return -1;
...
void *get_encode_data(void *attr) {
    ...
    if (enType == PT_H264) {
        sprintf(out_file, "sample_venc_ch%d.h264", vencChn);
        outFile = fopen(out_file, "wb");
    } else if (enType == PT_H265) {
        sprintf(out_file, "sample_venc_ch%d.h265", vencChn);
        outFile = fopen(out_file, "wb");
    } else {
        sprintf(out_file, "sample_venc_ch%d.jpg", vencChn);
        outFile = fopen(out_file, "wb");
    }
    // wait for set ready
    pthread_mutex_lock(&sample_attr->init_lock); 
